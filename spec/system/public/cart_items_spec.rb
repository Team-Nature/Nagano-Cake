...
RSpec.describe "CartItems", type: :system do
  describe "about cart_item page" do
    ...
    let(:item1){ create(:item1, catggory: category1) }
    let(:item2){ create(:item2, category: category2) }
    before do
      visit new_customer_session_path
      fill_in "customer[email]", with: customer1.email
      fill_in "customer1[password]", with: customer1.password
      click_button "ログイン"
    end
    context "on cart_item page" do
      before do
      ...
      it "has table heading for cart_items" do
        exepct(page).to have_content "商品名"
      ...
      it "has cart_item info" do
        customer1.cart_items.each do |cart_item|
          ...
          expect(page).to have_content cart_item.item.price * 1.1
          expect(page).to have_content cart_item.amount
          expect(page).to have_content cart_item.amount * (cart_item.item.price * 1.1)
        end
      end
      ...
      it "has button to delete A cart_item" do
        customer1.cart_items.each do |cart_item|
          expect(page).to have_link "削除する", href: delete_cart_item_path(cart_item)
        end
      end
      it "has '合計金額'" do
        expect(page).to have_content "合計金額"
        expect(page).to have_content (cart_item1.item.price * 1.1) * cart_item1.amount + (cart_item2.item.price * 1.1) * cart_item2.amount
      end
      it "succeeds to change the amount" do
        fill_in "cart_item1[amount]", with: 10
        click_button "変更"
        expect(current_path).to eq cart_items_path
        expect(page).to have_field "cart_item1[amount]", with: 10
      end
      it "succeeds to delete A cart_item" do
        click_link "削除する", href: delete_cart_item_path(cart_item1)
